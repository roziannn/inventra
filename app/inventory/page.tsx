...
import { IconPlus, IconSearch, IconFilter, IconEdit, IconPackage, IconHash, IconTags, IconDatabase, IconWeight, IconLayoutGrid, IconCircleCheck, IconCircleX } from "@tabler/icons-react";
...
export default function InventoryPage() {
  ...
  const columns = [
    ...
    {
      header: "Status",
      accessor: (item: any) => <span className={`text-[10px] font-bold uppercase tracking-wider px-2 py-1 rounded-md ${item.status === "In stock" ? "bg-emerald-100 text-emerald-700" : "bg-red-100 text-red-700"}`}>{item.status}</span>,
    },
    {
      header: "Aksi",
      accessor: (item: any) => (
      ...
      <div className="flex flex-col gap-4">
        ...
        <div className="flex flex-col md:flex-row gap-4 justify-between items-center py-2">
          <div className="relative w-full md:w-80 group">
            ...
            <input type="text" placeholder="Cari berdasarkan nama atau SKU..." className="w-full pl-7 pr-4 py-2 bg-transparent border-b border-gray-200 text-sm focus:outline-none focus:border-[#064E3B] transition-all" />
          </div>
        ...
        <Modal isOpen={isCategoryModalOpen} onClose={() => setIsCategoryModalOpen(false)} title="Manajemen Kategori Inventaris">
          <div className="space-y-6">
            <div className="p-4 bg-emerald-50 rounded-xl border border-emerald-100">
              <p className="text-sm text-emerald-800 leading-relaxed font-medium">Kelola kategori barang untuk memudahkan pengelompokan inventaris Anda.</p>
            </div>
